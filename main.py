...
# ===== المنقذ: إصلاح وربط الـ 80 مسلسل (الطريقة الآمنة) =====
...
async def fix_old_data(client, message):
    ...
    try:
        # استخدام async for المباشر لتجنب خطأ 400 BOT_METHOD_INVALID
        async for msg in client.get_chat_history(CHANNEL_ID):
            try:
                ...
                if is_video:
                    ...
                    # البحث عن البوستر الأصلي في آخر 50 رسالة قبل هذا الفيديو
