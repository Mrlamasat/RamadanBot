...
app = Client("PulsingDurationBot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def create_super_poster(base_path, duration_text, output="final_animation.gif"):
    try:
        ...
        for scale in scales:
            ...
            # إضافة شريط المعلومات السفلي (نمط Netflix)
...
# ===== استقبال الفيديو (جلب المدة بدون تحميل) =====
...
async def receive_video(client, message):
    ...
    if message.video:
        ...
        # جلب المدة إذا كانت متوفرة في الملف
    ...
    await message.reply_text(f"✅ تم الربط.\n⏱ المدة: {d_text}\nأرسل البوستر الآن.")
...
async def receive_poster(client, message):
    ...
    t_msg = await message.reply_text("⏳ جاري إنشاء بوستر النبض والمدة...")
...
# ===== فحص الاشتراك + تعديل العنوان =====
@app.on_message(filters.command("start") & filters.private)
async def start(client, message):
    f_id = message.command[1] if len(message.command) > 1 else None
    try:
        await client.get_chat_member(PUBLIC_CHANNEL, message.from_user.id)
        if f_id: await client.copy_message(message.chat.id, CHANNEL_ID, int(f_id))
        else: await message.reply_text("مرحباً بك يا محمد!")
    except UserNotParticipant:
        btn = [[InlineKeyboardButton("📢 اشترك أولاً", url=f"https://t.me/{PUBLIC_CHANNEL}")]]
        if f_id: btn.append([InlineKeyboardButton("✅ تم الاشتراك", callback_data=f"chk_{f_id}")])
        await message.reply_text("⚠️ اشترك لمشاهدة الحلقة.", reply_markup=InlineKeyboardMarkup(btn))

@app.on_callback_query(filters.regex(r"^edit_|^chk_"))
async def callbacks(client, query):
    data = query.data.split("_")
    if data[0] == "edit":
        video_memory[f"editing_{query.from_user.id}"] = {"mid": query.message.id, "vid": data[1]}
        await query.answer("📝 أرسل العنوان الجديد كرسالة نصية.")
    elif data[0] == "chk":
        try:
            await client.get_chat_member(PUBLIC_CHANNEL, query.from_user.id)
            await query.message.delete()
            await client.copy_message(query.from_user.id, CHANNEL_ID, int(data[1]))
        except: await query.answer("⚠️ لم تشترك بعد!", show_alert=True)
